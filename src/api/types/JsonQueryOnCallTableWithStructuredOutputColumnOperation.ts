// This file was auto-generated by Fern from our API Definition.

/**
 * This is the aggregation operation to perform on the column.
 * When the column is a structured output type, the operation depends on the value of the structured output.
 * If the structured output is a string or boolean, the operation must be "count".
 * If the structured output is a number, the operation can be "average", "sum", "min", or "max".
 */
export const JsonQueryOnCallTableWithStructuredOutputColumnOperation = {
    Average: "average",
    Count: "count",
    Sum: "sum",
    Min: "min",
    Max: "max",
} as const;
export type JsonQueryOnCallTableWithStructuredOutputColumnOperation =
    (typeof JsonQueryOnCallTableWithStructuredOutputColumnOperation)[keyof typeof JsonQueryOnCallTableWithStructuredOutputColumnOperation];
