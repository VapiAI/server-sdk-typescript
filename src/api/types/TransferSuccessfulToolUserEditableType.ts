// This file was auto-generated by Fern from our API Definition.

/** The type of tool. "transferSuccessful" for Transfer Successful tool. This tool can only be used during warm-transfer-experimental by the transfer assistant to confirm that the transfer should proceed and finalize the handoff to the destination. */
export const TransferSuccessfulToolUserEditableType = {
    TransferSuccessful: "transferSuccessful",
} as const;
export type TransferSuccessfulToolUserEditableType =
    (typeof TransferSuccessfulToolUserEditableType)[keyof typeof TransferSuccessfulToolUserEditableType];
