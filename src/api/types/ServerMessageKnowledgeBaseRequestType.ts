// This file was auto-generated by Fern from our API Definition.

/** This is the type of the message. "knowledge-base-request" is sent to request knowledge base documents. To enable, use `assistant.knowledgeBase.provider=custom-knowledge-base`. */
export const ServerMessageKnowledgeBaseRequestType = {
    KnowledgeBaseRequest: "knowledge-base-request",
} as const;
export type ServerMessageKnowledgeBaseRequestType =
    (typeof ServerMessageKnowledgeBaseRequestType)[keyof typeof ServerMessageKnowledgeBaseRequestType];
